...
import { FileText, TrendingUp, Clock, CheckCircle2, type LucideIcon } from "lucide-react";
...
function faturamentoPorMes(orcamentos: Orcamento[], meses = 6) {
  const hoje = new Date();
  const buckets = [];
  ...
  return (
    <Card className="flex items-center gap-3.5">
      ...
      <div>
        ...
        <div className="text-[19px] font-bold text-text">{value}</div>
      </div>
    </Card>
  );
}
...
export default function PainelPage() {
  ...
  const moeda: CodigoMoeda = empresaAtiva.moeda || "BRL";
  const hoje = todayStr();
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dadosGrafico} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(v) => formatMoney(v as number, moeda)}
                  contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12.5, color: "var(--text)" }}
                />
